...
export default function SettingsPageContent() {
  ...
  const resetTagError = useCallback(() => {
    setTagError(null);
  }, []);
              ...
              contexts.map((context) => {
                ...
                return (
                  <div key={context.id} className="space-y-2">
                    <div className="flex items-center justify-between gap-2 rounded-lg border border-border bg-surface-elevated/60 px-3 py-2 text-sm">
                      {isEditing ? (
                        <input
        ...
        {tagsOpen ? (
          <div className="mt-3 space-y-2">
            {tagsLoading ? (
              ...
              <p className="text-xs text-text-muted">{t("settings.noTags")}</p>
            ) : (
              tags.map((tag) => {
                        ...
                        {isEditing ? (
                          ...
                          <>
                            <button
                              type="button"
                              className="rounded-md px-2 py-1 text-xs font-medium text-text-secondary hover:bg-surface-elevated"
                              onClick={() => startTagEdit(tag.id, tag.name)}
                            >
                              {t("common.edit")}
                            </button>
                            <button
