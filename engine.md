...
## Помидор (Pomodoro)
- В нижней панели есть отдельная кнопка с иконкой помидора (слева от Play) для быстрого включения/выключения режима.
- Кнопка **«Настройка помидора»** размещена в разделе `/settings` и открывает модальное окно настроек.
...
- Кнопка помидора доступна всегда и включает/выключает режим без запуска основного таймера.
- На переходах фаз:
