...
- `/goals/new`:
  - поля цели (MVP-минимум):
    ...
    - target_value (counter) / время (time, ввод HH:MM)
...
- `/`:
  ...
  - отображение: title, period, context, tags, прогресс по текущему периоду
  - формат времени: `HH:MM:SS`
  - фильтры: контекст, теги, статус, Reset
  - кнопка `Add goal` в нижней панели справа
...
- Для `counter`:
  - инлайн-поле + кнопка `+`
  - если поле пустое, добавляется `1`
...
- `/goals/[id]`:
  - шапка цели + прогресс
  - список событий (MVP, DESC по времени, внутри скролла):
    - time: started_at – ended_at + duration, лимит 50
    - counter: occurred_at + +value_delta, лимит 50
    - check: occurred_at + state, лимит 50
  - форма ручного добавления:
    - `time`: start/end
    - `counter`: occurred_at + delta (> 0)
    - `check`: occurred_at + state
  - удаление событий (time/counter/check)
...
- Глобальная кнопка Play:
  ...
  - теги — опционально (goal исключён в MVP)
...
- Ограничение: одна активная сессия на пользователя
  ...
  - `Add goal` в правой части нижней панели
...
- фильтры на главной (multi-select):
  - контексты (single select)
  - теги (multi select)
  - статусы goal_periods (single select)
  - кнопка Reset
