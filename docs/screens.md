...
- Колесо мыши: прокрутка вниз → переход влево, вверх → переход вправо.
...
- Пока таймер активен, каждые 30 секунд показывается краткое уведомление, проигрывается звук около 2 секунд и (при разрешении) отправляется системное уведомление (через Service Worker, может зависеть от ограничений фонового режима браузера).
- При активном таймере доступна кнопка настройки «помидора» (Pomodoro) и рядом отображается обратный отсчёт фазы Фокус/Отдых, если режим включён.
