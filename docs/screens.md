...
- Колесо мыши: горизонтальная прокрутка (боковое колесо/тачпад) переключает страницы; вертикальная прокрутка страницу не меняет.
...
- Кнопка помидора (быстрое включение/выключение) расположена слева от Play и доступна всегда; настройки Pomodoro находятся в разделе /settings. Режим запускает обратный отсчёт автоматически при активном таймере, рядом отображается фаза Фокус/Отдых, если режим включён.
