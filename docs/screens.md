...
## Header (глобально)
- Кнопка настроек (шестерёнка) открывает меню управления: тема, язык, выход.
- В настройках есть разделы для управления контекстами и тегами (переименование/удаление с предупреждениями).
