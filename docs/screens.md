...
- При активном таймере доступна отдельная кнопка помидора (быстрое включение/выключение) и кнопка настройки «помидора» (Pomodoro); можно включать/выключать уведомления и звук, режим запускает обратный отсчёт автоматически, рядом отображается фаза Фокус/Отдых, если режим включён.
