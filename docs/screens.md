...
- Кнопка помидора (быстрое включение/выключение) и кнопка настройки «помидора» (Pomodoro) доступны всегда; можно включать/выключать уведомления и звук, режим запускает обратный отсчёт автоматически при активном таймере, рядом отображается фаза Фокус/Отдых, если режим включён.
